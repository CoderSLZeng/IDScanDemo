...
@interface AVCaptureViewController () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate> {
    IDCardScaningView *cardScaningView;
}
...
// 回调信息
@property (nonatomic, copy) IDScanSuccess successBlock;
...
- (void)viewDidLoad {
    ...
    cardScaningView = [[IDCardScaningView alloc] initWithFrame:self.view.frame];
    self.faceDetectionFrame = cardScaningView.facePathRect;
    [self.view addSubview:cardScaningView];
    ...
    //设置扫描范围
    //坐标转换
    self.metadataOutput.rectOfInterest = [self.previewLayer metadataOutputRectOfInterestForRect:cardScaningView.facePathRect];
    //    }];
...
//                
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (self.successBlock) {
                        self.successBlock(iDInfo, subImage);
                    }
                    [self.navigationController popViewControllerAnimated:YES];
                });
            }
